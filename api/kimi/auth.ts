...
// Lazy JWKS initialization — only created when needed, not at import time
let jwksInstance: ReturnType<typeof jose.createRemoteJWKSet> | null = null;

function getJwks() {
  if (!jwksInstance) {
    const jwksUrl = env.kimiAuthUrl || "https://auth.kimi.com";
    jwksInstance = jose.createRemoteJWKSet(
      new URL(`${jwksUrl}/api/.well-known/jwks.json`),
    );
  }
  return jwksInstance;
}
